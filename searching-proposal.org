...
#+BEGIN_EXAMPLE
  ...
  condition = general-condition | priority-condition | state-condidition | tag-condition | time-condition
            ,| composite-condition;

  general-value     = ? anything but a newline character ?;
  general-property  = "title" | "body" | "clocked" | "clocking"
                    ,| ? any non-whitespace character but cannot end with a plus sign. It cannot be the empty string. ?;
  general-condition = general-property, ".", operator, general-value;
  ...
  time-property    = "s" | "d" | "c" | "clockin" | "clockout";
  day              = 2 * digit;
  month            = 2 * digit;
  year             = 4 * digit;
  date             = year, "-", month, "-", day;
  date-time-stamp  = "now";
  time-period-unit = "d" | "w" | "m" | "y"
                   ,| "today" | "tod"
                   ,| "tomorrow" | "tmrw" | "tom";
  time-period      = [-], number, time-period-unit;
  time-value       = time-period | date-time-stamp | date | none;
  time-condition   = time-property, ".", operator, ".", time-value;
  ...
  state-class-value     = "todo" | "done" | none;
  ...
  composite-condidition   = condition, whitespace, binary-logical-operator, whitespace, condition
                          ,| condition, whitespace, condition
                          ,| "-", condition
                          ,| "not", whitespace, condition;

  condition-with-parens = "(", whitespace, condition, whitespace, ")";


  orderby-property    = "b" | "book" | "notebook"
                      ,| "s" | "sched" | "scheduled"
                      ,| "d" | "dead" | "deadline"
                      ,| "p" | "pri" | "prio" | "priority";
  ordering            = "o.", orderby-property;
  ordering-expression = ordering, {ordering};


  search = condition, [ordering-expression]
#+END_EXAMPLE
